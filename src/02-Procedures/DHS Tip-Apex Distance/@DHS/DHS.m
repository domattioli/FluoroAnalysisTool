classdef DHS < Procedure
    ...
    methods
        ...
        function result = compileResult( obj )
            ...
            if ~isempty( wire )
                ...
                if ~isempty( wire.get( 'Boundary' ) )
                    ...
                    wire_struct.XY = round( wireEQ( 0:.1:1 ), 2 );
                    wire_struct.PX_Width	= round( wire.get( 'WidthPX' ), 2);
                    wire_struct.MM_Width = wire.get( 'WidthMM' );
                end
        ...
        function [TAD, Theta] = evaluate( obj )
            ...
            femur   = obj.get( 'Femur' );
            wire    = obj.get( 'Wire' );
            TADNotReady = any( isnan( femur.get( 'TipApex' ) ) )...
                || isempty( wire.get( 'WidthPX' ) )...
                || isnan( wire.get( 'WidthPX' ) );
            if TADNotReady
                return
            end

            % Compute euclidean distance (in [pixels]) between wire tip and tip apex.
...
%             wireXY  = vertcat( wire.get( 'Base' ), wire.get( 'Tip' ) );
            ...
            % Compute angle between wire and femoral neck bisector.
