...
saveDirectoryName   = uigetdir(fullfile(directoryName,'..'), 'Select folder to save de-identified DICOM (.dcm) files.');
...
%% 1. Save All Dicoms in Specified Directory As De-Identified .dcm's
...
for idx = 1:nIter
    ...
    % Get all files in folder, regardless of extension.
    fileNames	= getFiles(fullfile(directoryName, folderNames{idx}));
    ...
    dicomNames  = strcat(fullfile(repmat({directoryName},...
         length(fileNames), 1), folderNames{idx}, fileNames), '.dcm');
    
    % De-Identify.
    success{idx} = deIdentifyDICOM(dicomNames, saveFolderName);
    if waitbarOn
